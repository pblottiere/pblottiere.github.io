...
(sent by the client) with the Python plugin and update the `LAYERS`{:.inline}
...
the usual `LAYERS`{:.inline} information.
...
updated accordingly: `LAYERS=T30UUU_20200901T112119_TCI`{:.inline}.

The Python implementation for such a basic need is very simple. Of course the
implementation may be improved to be more robust and generic.
...
class TimeFilter(QgsServerFilter):
    ...
    def requestReady(self):
        ...
        name = "{}_{}_TCI".format(params['LAYERS'], dt.toString("yyyyMMddThhmmss"))

        request.setParameter('LAYERS', name)
````
{% endcapture %}
