...
Basically, the idea is to catch and interpret the `TIME`{:.inline} parameter
(sent by the client) with the Python plugin and update the `LAYER`{:.inline}
parameter accordingly to retrieve the desired layers. This way the QGIS Server
process itself isn't even aware of the `TIME`{:.inline} parameter and only use
the usual `LAYER`{:.inline} information.

First things first, we use the
...
receives for example `TIME=2020-09-01 11:21:19`{:.inline}, the layer name is
...
<br/>Otherwise, in other cases, we don't want to have the naming constraint. Then
even if the underlying idea is the same, we have to adjust the mechanism
allowing to retrieve the correct layer's name. So the first thing to do is to
set up the temporal property on each layer. By the way, a script can easily do
it automatically in case of a lot of images.

<br/>
![OGC Badge]({{ site.baseurl }}/images/posts/wmst/temporal.png){:class="center-image" width="600px"}
<br/>

In the Python plugin, we will iterate over each layer of the group (instead of
all layers of the project to gain some speed) and use the temporal setting to
...
For now, QGIS Server is lacking an in-core solution for rasters and WMS-T but
in case of simple scenario, a Python plugin may implemented. However, it may
