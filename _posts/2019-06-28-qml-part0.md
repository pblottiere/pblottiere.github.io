...
method is also defined as a property to be able to call `pypie.slices`{:.inline} in the
QML file.
...
surface by municipaliy in Brittany, is used to define a Rule based renderer:
...
<br/>
Then, the `initSlices()`{:.inline} QML function is updated and a new
`sortValues()`{:.inline} function is introduced to sort slices by value:
